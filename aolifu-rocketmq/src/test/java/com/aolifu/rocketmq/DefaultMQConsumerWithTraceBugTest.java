...
public class DefaultMQConsumerWithTraceBugTest {
    ...
    private static final int MSG_NUM = 1000;

    @Before
    public void init() throws Exception {
        ConcurrentMap<String, MQClientInstance> factoryTable
            = (ConcurrentMap<String, MQClientInstance>) FieldUtils.readDeclaredField(MQClientManager.getInstance(),
            "factoryTable", true);
        ...
        mQClientFactory = spy(MQClientManager.getInstance()
            .getOrCreateMQClientInstance(pushConsumer,
        ...
        when(mQClientFactory.getMQClientAPIImpl()
            .pullMessage(anyString(), any(PullMessageRequestHeader.class), anyLong(), any(CommunicationMode.class),
            ...
            public PullResult answer(InvocationOnMock mock) throws Throwable {
                ...
                for (int i = 0; i < MSG_NUM; i++) {
                    ...
                    messageClientExt.setMsgId(i + "");
                    messageClientExt.setBody(new byte[] {'a'});
                    messageClientExt.setOffsetMsgId(i + "");
    ...
    public void testPullMessage_WithTrace_Success()
        throws InterruptedException, RemotingException, MQBrokerException, MQClientException {
        traceProducer.getDefaultMQProducerImpl()
            .getmQClientFactory()
            .registerProducer(producerGroupTraceTemp, traceProducer.getDefaultMQProducerImpl());
        ...
        pushConsumer.getDefaultMQPushConsumerImpl()
            .setConsumeMessageService(
                new ConsumeMessageConcurrentlyService(pushConsumer.getDefaultMQPushConsumerImpl(), (msgs, context) -> {
    ...
    private PullResultExt createPullResult(PullMessageRequestHeader requestHeader, PullStatus pullStatus,
        ...
        return new PullResultExt(pullStatus, requestHeader.getQueueOffset() + messageExtList.size(), 123, 2048,
            messageExtList, 0, outputStream.toByteArray());
    }
